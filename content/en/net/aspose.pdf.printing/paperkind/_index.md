...
source_url: "https://reference.aspose.com/pdf/net/aspose.pdf.printing/paperkind/"
generated_from: "online-reference"
fetched_at: "2026-06-14T04:38:32+00:00"
---
## PaperKind enumeration {#paperkind-enumeration}
...
| LegalExtra | `51` | Legal extra paper (9.275 in. by 15 in.). |
| TabloidExtra | `52` | Tabloid extra paper (11.69 in. by 18 in.). |
| A4Extra | `53` | A4 extra paper (236 mm by 322 mm). |
...
| JapaneseDoublePostcard | `69` | Japanese double postcard (200 mm by 148 mm). |
| A6 | `70` | A6 paper (105 mm by 148 mm). |
| JapaneseEnvelopeKakuNumber2 | `71` | Japanese Kaku #2 envelope. |
| JapaneseEnvelopeKakuNumber3 | `72` | Japanese Kaku #3 envelope. |
| JapaneseEnvelopeChouNumber3 | `73` | Japanese Chou #3 envelope. |
| JapaneseEnvelopeChouNumber4 | `74` | Japanese Chou #4 envelope. |
| LetterRotated | `75` | Letter rotated paper (11 in. by 8.5 in.). |
| A3Rotated | `76` | A3 rotated paper (420 mm by 297 mm). |
| A4Rotated | `77` | A4 rotated paper (297 mm by 210 mm). |
| A5Rotated | `78` | A5 rotated paper (210 mm by 148 mm). |
| B4JisRotated | `79` | JIS B4 rotated paper (364 mm by 257 mm). |
| B5JisRotated | `80` | JIS B5 rotated paper (257 mm by 182 mm). |
| JapanesePostcardRotated | `81` | Japanese rotated postcard (148 mm by 100 mm). |
| JapaneseDoublePostcardRotated | `82` | Japanese rotated double postcard (148 mm by 200 mm). |
| A6Rotated | `83` | A6 rotated paper (148 mm by 105 mm). |
| JapaneseEnvelopeKakuNumber2Rotated | `84` | Japanese rotated Kaku #2 envelope. |
| JapaneseEnvelopeKakuNumber3Rotated | `85` | Japanese rotated Kaku #3 envelope. |
| JapaneseEnvelopeChouNumber3Rotated | `86` | Japanese rotated Chou #3 envelope. |
| JapaneseEnvelopeChouNumber4Rotated | `87` | Japanese rotated Chou #4 envelope. |
| B6Jis | `88` | B6 envelope (176 mm by 125 mm). |
| B6JisRotated | `89` | JIS B6 rotated paper (182 mm by 128 mm). |
| Standard12x11 | `90` | Standard paper (12 in. by 11 in.). |
| JapaneseEnvelopeYouNumber4 | `91` | Japanese You #4 envelope. |
| JapaneseEnvelopeYouNumber4Rotated | `92` | Japanese You #4 rotated envelope. |
| Prc16K | `93` | 16K paper (146 mm by 215 mm). |
| Prc32K | `94` | 32K paper (97 mm by 151 mm). |
| Prc32KBig | `95` | 32K big paper (97 mm by 151 mm). |
| PrcEnvelopeNumber1 | `96` | #1 envelope (102 mm by 165 mm). |
| PrcEnvelopeNumber2 | `97` | #2 envelope (102 mm by 176 mm). |
| PrcEnvelopeNumber3 | `98` | #3 envelope (125 mm by 176 mm). |
| PrcEnvelopeNumber4 | `99` | #4 envelope (110 mm by 208 mm). |
| PrcEnvelopeNumber5 | `100` | #5 envelope (110 mm by 220 mm). |
| PrcEnvelopeNumber6 | `101` | #6 envelope (120 mm by 230 mm). |
| PrcEnvelopeNumber7 | `102` | #7 envelope (160 mm by 230 mm). |
| PrcEnvelopeNumber8 | `103` | #8 envelope (120 mm by 309 mm). |
| PrcEnvelopeNumber9 | `104` | #9 envelope (229 mm by 324 mm). |
| PrcEnvelopeNumber10 | `105` | #10 envelope (324 mm by 458 mm). |
| Prc16KRotated | `106` | 16K rotated paper (146 mm by 215 mm). |
| Prc32KRotated | `107` | 32K rotated paper (97 mm by 151 mm). |
| Prc32KBigRotated | `108` | 32K big rotated paper (97 mm by 151 mm). |
| PrcEnvelopeNumber1Rotated | `109` | #1 rotated envelope (165 mm by 102 mm). |
| PrcEnvelopeNumber2Rotated | `110` | #2 rotated envelope (176 mm by 102 mm). |
| PrcEnvelopeNumber3Rotated | `111` | #3 rotated envelope (176 mm by 125 mm). |
| PrcEnvelopeNumber4Rotated | `112` | #4 rotated envelope (208 mm by 110 mm). |
| PrcEnvelopeNumber5Rotated | `113` | #5 rotated envelope (220 mm by 110 mm). |
| PrcEnvelopeNumber6Rotated | `114` | #6 rotated envelope (230 mm by 120 mm). |
| PrcEnvelopeNumber7Rotated | `115` | #7 rotated envelope (230 mm by 160 mm). |
| PrcEnvelopeNumber8Rotated | `116` | #8 rotated envelope (309 mm by 120 mm). |
| PrcEnvelopeNumber9Rotated | `117` | #9 rotated envelope (324 mm by 229 mm). |
| PrcEnvelopeNumber10Rotated | `118` | #10 rotated envelope (458 mm by 324 mm). |
